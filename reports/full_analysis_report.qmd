...
# Use plot_results from pipeline run to get only the current 10 compact plots
plot_results <- result$phase_results$plot_generation$results

# Extract paths for compact plots (indices 1-10)
compact_paths <- sapply(plot_results[1:10], function(x) x$output_path %||% NA)
compact_paths <- compact_paths[!is.na(compact_paths) & file.exists(compact_paths)]

if (length(compact_paths) > 0) {
  knitr::include_graphics(compact_paths)
} else {
  cat("No compact plots found. Run the pipeline to generate plots.\n")
}
```
...
# Extract paths for expanded plots (indices 11-20)
expanded_paths <- sapply(plot_results[11:20], function(x) x$output_path %||% NA)
expanded_paths <- expanded_paths[!is.na(expanded_paths) & file.exists(expanded_paths)]
