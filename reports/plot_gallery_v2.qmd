...
# Render plot by ID (e.g., "compact_01")
render_plot <- function(plot_id, alt_text = "Plot not available") {
  if (plot_id %in% names(plot_file_lookup)) {
    path <- plot_file_lookup[[plot_id]]$file_path
...
# Get palettes by grouping plots
plots_by_palette <- get_plots_grouped(group_by = "fill", plot_type = "ridgeline")

# Create tabs for each palette (limit to one of each fill color)
...
for (fill in unique_fills) {
  ...
  if (!is.null(config_compact) && !is.null(config_expanded)) {
    ...
    # Compact
    cat("```{r}\n")
    cat(sprintf("render_plot('%s')\n", config_compact$id))
    cat("```\n")
    cat(sprintf("**Compact (0.85)**  \n")
    cat(sprintf("Variant %d | Quality: %.0f/100\n\n", 
         which(names(plot_registry$ridgeline$variants) == config_compact$id),
         get_plot_info(config_compact$id)$quality))
    ...
    cat(sprintf("Variant %d | Quality: %.0f/100\n\n",
         ...
         get_plot_info(config_expanded$id)$quality))
